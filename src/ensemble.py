...
from . import euclidean_distance, manhattan_distance, cosine_similarity
...
class CustomKMeans():
    ...
    def assign_clusters(self, data):
        ...
        for n in range(self.n_clusters):
            distances.append(self.metric_fn(data, self.cluster_centers_[n]))
    ...
    def update_cluster_centers(self, data):
        ...
        cluster_centers = []
        for n, label in enumerate(range(self.n_clusters)):
            if np.any(self.labels_ == label):
                cluster_centers.append(data[self.labels_ == label, :].mean(axis = 0))
            else:
                cluster_centers.append(data[:, self.metric_fn(data, self.cluster_centers_[n]).argmax()])
    ...
    def fit(self, data):
        ...
        if self.init == 'k-means++':
            self.cluster_centers_, _ = kmeans_plusplus(data[X_name_num].values, n_clusters = self.n_clusters)
        elif self.init == 'random':
            self.cluster_centers_ = data[np.random.randint(0, data.shape[0], (self.n_clusters, data.shape[1]))]
        
        new_labels_ = self.assign_clusters(data, self.cluster_centers_)
