...
class Polynomial(nn.Module):
    def __init__(self, degree = 1, coef_init = None, coef_train = True,
        ...
        # The shape of coef depends on degree and whether zero_order term is included
        # This initialization assumes coef is shared across the specified dim
        coef_shape = (degree + int(zero_order),)
        if coef_init is None:
            coef_init = torch.nn.init.normal_(torch.empty(coef_shape,
                                                          device = device, dtype = dtype))
        
        self.coef = torch.nn.Parameter(data = coef_init, 
                                       requires_grad = coef_train)
    ...
    def forward(self, X):
        ...
        # Generate powers
        pows = torch.arange(1 - int(self.zero_order), (self.degree + 1), 
                            device = self.device, dtype = self.dtype)
        
        # Adjust shape for broadcasting the operation along the specified dim
        # Unsqueeze input X and powers for broadcasting
        X_pow = X.unsqueeze(self.dim + 1)
        pows = pows.unsqueeze(0)
        
        # Polynomial transformation
        transformed = (X_pow.pow(pows) * self.coef).sum(dim = self.dim + 1)
