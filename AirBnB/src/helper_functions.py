...
def calculate_inquiry_rate(df, variable = None, value = None, interval = 'D'):
    ...
    df_value = df[df[variable] == value]
    df_value_resampled = df_value.resample(interval).agg({'id_guest_anon': 'nunique'})

    value_inquiry_rate = (df_value_resampled['id_guest_anon'] / df_resampled_total['id_guest_anon'])
...
def calculate_listing_rate(df, variable = None, value = None, interval = 'D'):
    ...
    df_value = df[df[variable] == value] if (variable is not None) & (value is not None) else df
    df_value_resampled = df_value.resample(interval).agg({'id_listing_anon': 'nunique'})
